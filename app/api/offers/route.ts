...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = new URL(`https://api.partner.market.yandex.ru/v2/campaigns/${campaignId}/offers`);
    ...
    if (body.categoryIds) requestBody.categoryIds = body.categoryIds;
    if (body.offerIds) requestBody.offerIds = body.offerIds;
    if (body.statuses) requestBody.statuses = body.statuses;
    if (body.tags) requestBody.tags = body.tags;
    if (body.vendorNames) requestBody.vendorNames = body.vendorNames;
